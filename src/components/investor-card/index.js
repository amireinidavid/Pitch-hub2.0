...
function PitchCard({ pitchItem, isLoading }) {
  const [isHovered, setIsHovered] = useState(false);

  // If the pitch is not active, don't render anything
  if (!isLoading && pitchItem?.status !== "active") {
    return null;
  }

  const getPitchStatusColor = (status) => {
    const colors = {
  ...
  return (
    ...
    >
      <Link href={`/pitches/${pitchItem?._id}`}>
          ...
          <CardHeader className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              >
                <Badge
                  variant="outline"
                  className={`${getPitchStatusColor(pitchItem?.status)}`}
                >
                  {pitchItem?.status?.charAt(0).toUpperCase() +
                    pitchItem?.status?.slice(1) || "Draft"}
                </Badge>
              </motion.div>
              <Avatar className="h-9 w-9 ring-2 ring-primary/20 transition-all duration-300 group-hover:ring-primary/40">
                <AvatarImage
                  src={pitchItem?.creator?.image}
                  alt={pitchItem?.creator?.name}
                />
                <AvatarFallback>
                  {pitchItem?.creator?.name?.slice(0, 2).toUpperCase()}
                </AvatarFallback>
              </Avatar>
            </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              >
                ...
                <span>{pitchItem?.creator?.company || "Startup"}</span>
              </motion.div>
              <motion.div
                className="flex items-center gap-2"
                whileHover={{ scale: 1.05 }}
              >
                ...
                <span>${pitchItem?.fundingGoal?.toLocaleString()}</span>
              </motion.div>
              <motion.div
                className="flex items-center gap-2"
                whileHover={{ scale: 1.05 }}
              ...
              >
                ...
                <span>{pitchItem?.applicationCount || 0} Interested</span>
              </motion.div>
            </div>
